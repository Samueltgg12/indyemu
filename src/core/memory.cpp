...
void Memory::reset() {
    ...
    const u32 entrylo_valid = (0 << 3) | (0 << 2) | (1 << 1) | (1 << 0); // C=0, D=0, V=1, G=1

    // Helper to set up a TLB entry
    auto setup_tlb_entry = [this, &pagemask_4mb, &entrylo_valid](u32 vaddr, u32 paddr, u32 index) {
        tlb_index_ = index;
    ...
    // Map PROM region in kuseg: virtual 0x1FC00000-0x1FDFFFFF -> physical 0x3FC00000-0x3FDFFFFF
    // Map PROM region in kseg0: virtual 0x9FC00000-0x9FDFFFFF -> physical 0x3FC00000-0x3FDFFFFF
    // Map PROM region in kseg1: virtual 0xBFC00000-0xBFDFFFFF -> physical 0x3FC00000-0x3FDFFFFF
    ...
    setup_tlb_entry(vbase_kuseg_prom, pbase_prom, num_ram_entries);         // index: RAM entries
    setup_tlb_entry(vbase_kseg0_prom, pbase_prom, num_ram_entries + 1);     // index: RAM entries + 1
    setup_tlb_entry(vbase_kseg1_prom, pbase_prom, num_ram_entries + 2);     // index: RAM entries + 2
    ...
    for (u32 i = 0; i < 20; ++i) {
        std::cerr << "  [" << i << "] entry_lo0=0x" << std::hex << std::setw(8) << std::setfill('0') << tlb_entries_[i].entry_lo0
                  << ", entry_lo1=0x" << std::hex << std::setw(8) << tlb_entries_[i].entry_lo1
                  << ", entry_hi=0x" << std::hex << std::setw(8) << tlb_entries_[i].entry_hi
                  << ", page_mask=0x" << std::hex << std::setw(8) << tlb_entries_[i].page_mask
                  << std::dec << std::endl;
    }
}
...
void Memory::write8(u32 address, u8 value) {
    ...
    if (isIoPhysicalAddress(paddr)) {
        ...
        io_bus_.write32(aligned, (current & ~mask) | (static_cast<u32>(value) << shift));
    }
}
...
void Memory::tlb_probe(u32 vaddr) {
    ...
    for (u32 i = 0; i < kTLBEntries; ++i) {
        ...
        bool global_bit_set = ((entry.entry_lo0 & 0x1u) || (entry.entry_lo1 & 0x1u));
...
u32 Memory::tlb_translate(u32 vaddr, bool& found, bool& invalid) const {
    ...
    for (u32 i = 0; i < kTLBEntries; ++i) {
        ...
        u32 mask = ~(entry.page_mask & 0xFFFFE000u);  // Mask out the bits that don't matter for VPN comparison
        ...
        // Check if the Global bit is set in either entry_lo0 or entry_lo1
        bool global_bit_set_in_entry = ((entry.entry_lo0 & 0x1u) || (entry.entry_lo1 & 0x1u));
        ...
        if (entry_vpn2 == vpn2 && asid_match) {
            ...
            // We need to figure out which page table entry to use based on the address
            // and the page mask

            // Calculate the page size from the page mask
            // PageMask encodes the page size as: (~(PageMask >> 13) & 0x3FFF) + 1) * 2KB
            ...
            // This depends on how the TLB splits the page - typically for odd/even pages
            // within a larger page frame
            ...
            invalid = !(use_lo1 ? (entry.entry_lo1 & 0x2) : (entry.entry_lo0 & 0x2));  // Valid bit
...
bool Memory::checkAndClearTLBException(u32& vaddr, bool& is_store, TLBExceptionType& type) {
...
Memory::TLBExceptionType Memory::tlb_exception_type_ = Memory::TLBExceptionType::kNone;
